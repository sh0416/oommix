...
from data import CollateFn, create_test_dataset
...
def convex_hull(points, graphic=False, smidgen=0.0075):
    ...
    while prev_pts > n_pts:
        ...
        while i < (n_pts - 2):
            ...
            if graphic:
                _draw_triangle(
                    centre, pts[i], pts[(i + 1) % n_pts], facecolor="blue", alpha=0.2
                )
                _draw_triangle(
                    centre,
                    pts[(i + 1) % n_pts],
...
def probe(model, loader, device):
        ...
        for batch in loader:
            ...
            rows = {
                "context_idx": [
                    i.item()
                    for i in batch["idx"]
                    for _ in range(batch["inputs"]["input_ids"].shape[1])
                ],
                "token_idx": [
                    x.item() for l in batch["inputs"]["input_ids"] for x in l
                ],
            }
    ...
    for name, l in activations.items():
        np.savetxt(
            name + ".csv", np.vstack(l), fmt="%.4f", delimiter=",", encoding="utf8"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, choices=["ag_news", "yahoo_answer"], default="ag_news"
    )
    ...
    parser.add_argument(
        "--augment",
        type=str,
        choices=["none", "tmix", "adamix", "proposed"],
        default="none",
    )
